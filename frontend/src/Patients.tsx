...
export default function Patients({ onSelect }: { onSelect: (patient: Patient) => void }) {
  ...
  return <>
    ...
    {register && <RegisterPatient onCancel={() => setRegister(false)} onCreated={patient => { setRegister(false); onSelect(patient) }} />}
    <Panel>
...
function RegisterPatient({ onCancel, onCreated }: { onCancel: () => void; onCreated: (patient: Patient) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setBusy(true)
    try { onCreated(await api<Patient>('/patients', { method: 'POST', body: { ...values, phone: values.phone || undefined, synthetic: true } })) } catch (error) { setError(message(error)) } finally { setBusy(false) }
  }
  return <Panel className="registration-panel">
    <div className="panel-heading panel-heading-row"><div><h2>Register a synthetic patient</h2><p>Use fictional details only in this research preview.</p></div><button className="icon-button" aria-label="Close registration" onClick={onCancel}><X size={20} /></button></div>
