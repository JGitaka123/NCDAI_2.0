...
Every function instance can create its own SQLAlchemy connection pool. Size the combined maximum across concurrent instances against the database connection allowance, rather than sizing only one process. The initial engine uses SQLAlchemy's normal pool defaults; host-specific bounded pool settings and load measurements must be confirmed before increasing traffic. `pool_pre_ping` checks reused connections. A PostgreSQL connection has a five-second connection timeout, fifteen-second statement timeout and five-second lock timeout in the application engine.

A provider's pooled application connection can reduce direct-connection pressure. Neon's pooler uses transaction pooling, which restricts session-dependent features; use the direct endpoint for migrations, dump/restore and administrative work. Audit appends use transaction-level advisory locks. Verify the application's timeout startup options, transaction locks and concurrent immutable review behavior against the actual selected pooled endpoint; compatibility is not inferred from a successful direct connection. Keep transactions short and do not retain one while waiting for AI. See [Neon connection pooling](https://neon.com/docs/connect/connection-pooling).

## Smoke check and restart

After every deployment, check the root page, bundled scripts/styles and both `/api/health/live` and `/api/health/ready` over the canonical HTTPS domain. Liveness identifies the running process; readiness checks database access and clinical evidence. Readiness does not independently compare the installed migration to Alembic head, so keep the release migration check. API failures must return API errors rather than the frontend HTML page.
