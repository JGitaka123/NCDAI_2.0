...
- Deployed commit: `1ded07988c4086ec75bf512e828410993a095e7b`. This record is published in the later commit `cae5152f2bd843c05822a0b7031f45e6dd769d6a`, which changes documentation, tests and scripts only; no `backend/app` or `frontend/src` source differs between the two. All three quality jobs also passed at that later commit ([run 34740464820](https://github.com/JGitaka123/NCDAI_2.0/actions/runs/34740464820)), and deployment is explicit rather than automatic, as recorded in the [deployment record](deployment.md).
...
- 66 general case workflows on SQLite and PostgreSQL, 75 dose workflows on PostgreSQL at the deployed dose version `ncdai-dose-reference-0.1.1` ([evidence](test-results/clinical-readiness-dose-postgres.json); the earlier [0.1.0 run](test-results/dose-cases-postgres.json) is superseded), and 50 complete consultant workflows using two PostgreSQL databases and restricted runtime roles passed. Repeated database runs are not additional distinct clinical cases. The consultant suite includes two concurrency checks.
...
Neon management currently requires the owner's email verification before opening the console. The linked Free plan is confirmed, but its actual backup/PITR retention setting has not been inspected. Complete that verification and confirm retention/recovery arrangements before relying on this service as the sole record of patient care. Until it is confirmed, the hospital's existing record-keeping must continue alongside the application for every clinical-testing encounter, so that no patient record exists only in this service. This is a concrete remaining operational check, not a new regulatory approval request.
