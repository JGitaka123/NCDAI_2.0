...
|---|---|---|
| Cloud backend regression | 228 passed, 8 optional live-provider tests skipped; no failures | [Quality run at 6415fed](https://github.com/JGitaka123/NCDAI_2.0/actions/runs/34709332691). Includes clinical boundaries, security, AI contracts and FHIR export checks; do not add overlapping test subsets to this count. |
...
| Final session-expiry correction | 10 timezone regressions passed; expired real-PostgreSQL session correctly rejected | [Release audit RA-01](release-audit.md). This follow-up fix is newer than the first cloud run above and requires the subsequent final-head CI run. |
...
Vercel deployment must have a dedicated PostgreSQL database, applied migrations, restricted application credentials, explicit HTTPS origin, secure session settings and a privately provisioned account. Hosted health, sign-in, persistence and a complete synthetic browser workflow must pass before the deployment is described as ready. A green CI run alone is insufficient.
